...
export interface LevelDef {
  ...
  milks: MilkId[];
  takeaway: boolean;
  takeawayChance: number;
  extraShot: boolean;
  extraShotChance: number;
  extraHot: boolean;
  extraHotChance: number;
  orderCount: number;
  queueLength: number;
  patience: boolean;
...
function learnLevel(drink: DrinkId, index: number): LevelDef {
  ...
  return {
    ...
    mode: 'learn',
    drinkPool: [drink],
    sizes: [recipe.allowedSizes[0] as SizeId],
    milks: ['whole'],
    ...NONE,
    orderCount: 1,
    queueLength: 1,
    ...
    multiDrink: false,
    guided: true,
    goal: `Learn to make a ${recipe.name.toLowerCase()}.`,
  };
}
...
function practiceLevel(drink: DrinkId, index: number): LevelDef {
  ...
  return {
    ...
    mode: 'practice',
    drinkPool: [drink],
    sizes: [...recipe.allowedSizes],
    milks: ['whole', 'semi-skimmed'],
    ...PRACTICE_MODS,
    orderCount: 3,
    queueLength: 2,
    patience: false,
    ...
    multiDrink: false,
    guided: false,
    goal: `Serve three ${recipe.name.toLowerCase()} orders with modifiers.`,
  };
}
...
function shiftLevel(
  ...
  return {
    ...
    sizes,
    milks,
    takeaway: (flags.takeaway ?? 0) > 0,
    takeawayChance: flags.takeaway ?? 0,
    extraShot: (flags.extraShot ?? 0) > 0,
    extraShotChance: flags.extraShot ?? 0,
    extraHot: (flags.extraHot ?? 0) > 0,
    extraHotChance: flags.extraHot ?? 0,
    orderCount,
    queueLength,
    patience: patienceSeconds > 0,
    patienceSeconds,
    timeScoring,
    parallelPrep: flags.parallelPrep ?? false,
    orderChanges: flags.orderChanges ?? false,
    multiDrink: flags.multiDrink ?? false,
    guided: false,
    goal,
  };
}
...
export const LEVELS: LevelDef[] = [
  ...
  shiftLevel(
    ...
    true,
    { takeaway: 0.3 },
    'Takeaway orders need the takeaway cup.',
  ),
  shiftLevel(
    'S6',
    ['espresso', 'americano', 'latte'],
    ...
    55,
    true,
    { takeaway: 0.3, extraShot: 0.2, extraHot: 0.15 },
    'Extra shots and large sizes join the menu.',
  ),
  shiftLevel(
    'S7',
